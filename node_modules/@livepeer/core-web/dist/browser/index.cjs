...
__export(browser_exports, {
  ...
  addMediaMetrics: () => addMediaMetrics,
  canPlayMediaNatively: () => canPlayMediaNatively,
        ...
        try {
          ...
          if (response.status === 404) {
            ...
            return store.getState().__controlsFunctions?.onError?.(
              new Error(import_errors2.STREAM_OFFLINE_ERROR_MESSAGE)
            );
          }
          if (response.status === 401) {
            console.warn("Unauthorized to view video");
            return store.getState().__controlsFunctions?.onError?.(
              new Error(import_errors2.ACCESS_CONTROL_ERROR_MESSAGE)
            );
          }
        } catch (err) {
          console.warn(err);
    ...
    async ({
      ...
      if (source.type === "webrtc") {
        const unsubscribeBframes = store.subscribe(
          ...
          (bframes) => {
            if (bframes) {
              onErrorComposed(new Error(import_errors2.BFRAMES_ERROR_MESSAGE));
            }
          }
        );
      ...
      if (source.type === "hls") {
        ...
        const onErrorCleaned = (error) => {
          const cleanError = new Error(
            error?.response?.data?.toString?.() ?? (error?.response?.code === 401 ? import_errors2.ACCESS_CONTROL_ERROR_MESSAGE : "Error with HLS.js")
          );
  ...
  const destroyPictureInPicture = store.subscribe(
    ...
    async () => {
      try {
        ...
        (0, import_utils6.warn)(e?.message ?? "Picture in picture is not supported");
...
var import_media2 = require("@livepeer/core/media");
...
function addMediaMetrics(element, opts = {}) {
  if (element) {
    ...
    const { store, destroy } = (0, import_media2.createControllerStore)({
      src: source,
      playbackId: opts?.playbackId,
      device: getDeviceInfo(import_version.version.core),
      storage: (0, import_storage.createStorage)({ storage: import_storage.noopStorage }),
      initialProps: {
    ...
    const { metrics, destroy: destroyMetrics } = (0, import_media2.addMediaMetricsToStore)(store, {
